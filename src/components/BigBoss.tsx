...
export default function BigBoss({ setActiveTab, isDark, toggleDark }: BigBossProps) {
  ...
  const openGoogleOAuthPopup = () => {
    const clientId = (import.meta as any).env?.VITE_GOOGLE_CLIENT_ID || "888780289762-gpiud6mhos00kiljpgnk779tunli4ijr.apps.googleusercontent.com";
    ...
    if (!popup) {
      showToast("Pop-up diblokir browser. Harap izinkan pop-up untuk login.", "error");
    }
  };
  ...
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      if (event.data && event.data.type === "TASKWAI_GOOGLE_ID_TOKEN" && event.data.idToken) {
        setShowLoginModal(false);
        await handleGoogleLoginResponse({ credential: event.data.idToken });
      }
    };
    ...
    if (window.location.hash.includes("id_token=")) {
      ...
      if (idToken && window.opener) {
        window.opener.postMessage({ type: "TASKWAI_GOOGLE_ID_TOKEN", idToken }, "*");
        window.close();
      }
    }
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center py-1">
                <button
                  ...
                  className="w-full max-w-xs py-3 px-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-300/80 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-100 font-bold text-xs flex items-center justify-center gap-3 shadow-sm transition-all cursor-pointer hover:border-emerald-500/60 focus:outline-none"
                >
                  ...
                  <span>{t("auth.googleSignIn", "Masuk dengan Google")}</span>
                </button>
              </div>
