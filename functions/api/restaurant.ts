...
export async function onRequest(context: any): Promise<Response> {
  ...
  try {
    ...
    if (path.endsWith('/api/restaurant/reset') && request.method === 'POST') {
      ...
      await db.prepare(
        `UPDATE expenses SET 
          sewaTempat = 0, gajiKaryawan = 0, royaltiFranchise = 0, listrik = 0, air = 0, 
          internet = 0, marketing = 0, pajak = 0, biayaLain = 0, cicilanBank = 0, updatedAt = ? 
         WHERE restaurantId = ?`
      ).bind(nowStr, restaurantId).run();
